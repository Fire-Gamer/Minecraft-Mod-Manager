...
class InstanceManager(object):
	...
	def create_instance(self, name: str, setting: bool, version: str, loader: str, mods: dict = {}, check: bool = True) -> None:
		...
		if check:
			if os.path.exists(f"{INSTANCES_FOLDER}/{name}.instance"):
				print(f"[ERROR] [INFO] An instance with the name of {name} already exists"); return None
		...
		with open(f"{INSTANCES_FOLDER}/{name.lower()}.instance", "w") as ins, open(f"{INDEX}", "a") as index:
			ins.write(save)
			index.write(f"{instance.get('name')}\n")
		os.makedirs(f"{DIF}\{name}")
		os.makedirs(f"{DIF}\{name}\mods")
		if setting == True: shutil.copy(f"{DMF}\options.txt", f"{DIF}\{name}\\")
	...
	def get_instance_file(self, name: str):
		with open(f"{INDEX}", "r") as index:
			...
			for instance in instance_names:
				if instance.lower() == name.lower() and os.path.exists(f"{INSTANCES_FOLDER}/{name.lower()}.instance"):
	...
	def read_instance(self, name: str):
		...
		with open(f"{INSTANCES_FOLDER}/{name.lower()}.instance", "r") as instance:
			for line in instance.readlines(): temp.append(line.strip().replace(",", ""))
		for i in ["[", "]", ""]:
			if i in temp: temp.remove(i)
		for i in temp[2:-4]:
			mods.update({i.strip().split(":")[0]: i.strip().split()[1].replace("'", "")})
		instance_dict.update({"name": temp[0].split("==")[1].capitalize()})
		instance_dict.update({"mods": mods})
		instance_dict.update({"setting": True if temp[-3].split("==")[1].lower() == "true" else False})
